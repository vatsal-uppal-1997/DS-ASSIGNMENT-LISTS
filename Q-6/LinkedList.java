...
public class LinkedList <T extends Comparable<T>> {
	...
	public void sort(Comparator<T> comp) {
		...
		for (int i=0; i < this.size; i++) {
			temp = this.start;
			for (int j=0; j < this.size-i-1; j++) {
				if (comp.compare(temp.getData(), (temp.getNext()).getData()) > 0) { // If temp is even and temp.getNext() is odd swap the two
						swap(temp, temp.getNext()); 				
				}
				temp = temp.getNext();
			}
	...
	public boolean isPalindromeEasy() {
	...
	public boolean isPalindromeHard() {
		int size = this.size();
	...
	public void merge(LinkedList<T> list2) {
		Node listTwoHead = list2.getNode(0);
		Node listOneHead = this.start;

		while(listTwoHead != null) {
			while (listOneHead != null && (listOneHead.getData()).compareTo(listTwoHead.getData()) < 0) {
				listOneHead = listOneHead.getNext(); // move until listOneHead is smaller than listTwoHead
			}
			if (listOneHead != null) 
				insertBefore(listOneHead,listTwoHead.getData()); // Found a node greater than listTwoHead, insert data before it
			else
				add(listTwoHead.getData()); // list One just ended just append stuff to it
			if (listOneHead != null)			
				listOneHead = this.start; // no need to start again once first linked list has ended, all elements are greater afterwards
	...
	public void insertBefore(Node element, T data) {		
		if (element == this.start)	{
			Node temp = new Node(data,this.start);
			this.start = temp;
			return;
		}	
		Node previous = getPrevious(element);
		Node temp = new Node(data, element);
		previous.setNext(temp);	
	}
	...
	public void insertAfter(Node element, T data) {
		Node temp = new Node(data, element.getNext());
